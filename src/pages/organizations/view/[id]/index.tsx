...
import { deleteTrainingById } from 'apiSdk/trainings';
...
function OrganizationViewPage() {
  ...
  const trainingHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('training', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/trainings/create?organization_id=${data?.id}`}>
                    ...
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.training?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/trainings/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
                            ...
                            <Td>
                              ...
                              {hasAccess('training', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    trainingHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
